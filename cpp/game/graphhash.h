...
namespace GraphHash {
  ...
  Hash128 getStateHash(const BoardHistory& hist, Player nextPlayer, double noResultUtilityForWhite);
  ...
  Hash128 getGraphHash(Hash128 prevGraphHash, const BoardHistory& hist, Player nextPlayer, int repBound, double noResultUtilityForWhite);

  //Compute graph hash from scratch by replaying the whole history.
  Hash128 getGraphHashFromScratch(const BoardHistory& hist, Player nextPlayer, int repBound, double noResultUtilityForWhite);
}
