...
namespace GraphHash {
  ...
  Hash128 getGraphHash(const BoardHistory& hist, Player nextPlayer, double drawEquivalentWinsForWhite);

  //Compute graph hash from scratch by replaying the whole history.
  Hash128 getGraphHashFromScratch(const BoardHistory& hist, Player nextPlayer,  double drawEquivalentWinsForWhite);
}
