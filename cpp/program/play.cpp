...
FinishedGameData* Play::runGame(
  ...
  if(playSettings.initGamesWithPolicy && otherGameProps.allowPolicyInit) {
    double proportionOfPolicyInit = otherGameProps.isSgfPos ? playSettings.startPosesPolicyInitProp : playSettings.policyInitProp;
    if(proportionOfPolicyInit > 0) {
      //Perform the initialization using a different noised komi, to get a bit of opening policy mixing across komi
      {
        ...
        PlayUtils::initializeGameUsingPolicy(botB, botW, board, hist, pla, gameRand, proportionOfPolicyInit, temperature);
