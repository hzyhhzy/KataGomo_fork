...
namespace NNUE {
  ...
  class MCTSsearch {
   public:
    NNUEHashTable* hashTable;
    MCTSnode* rootNode;
    Color board[MaxBS * MaxBS];
    Hash128 posHash;  // only board
    NNUE::ExtraStates states;

    Evaluator evaluator;
    VCFsolver_NNUE vcfSolver[2];
    ...
    MCTSsearch(const NNUEV2::ModelWeight* w, NNUEHashTable* ht);
   ...
   private:
    ...
    friend MCTSnode::MCTSnode(MCTSsearch* search, Color nextColor, double policyTemp);
    struct SearchResult {
      uint64_t newVisits;
