...
class Model:
  ...
  def build_model(self,config,placeholders):
    ...
    p2_layer = self.conv_only_block("p2",p1_layer,diam=1,in_channels=p1_num_channels,out_channels=Model.NUM_POLICY_TARGETS,scale_initial_weights=0.3)
    ...
    self.p2_conv = ("p2",1,p1_num_channels,Model.NUM_POLICY_TARGETS)
...
class Target_vars:
  def __init__(self,model,for_optimization,placeholders):
    ...
    self.policy_target1 = (placeholders["policy_target1"] if "policy_target1" in placeholders else
                          tf.compat.v1.placeholder(tf.float32, [None] + model.policy_target_shape))
    self.policy_target2 = (placeholders["policy_target2"] if "policy_target2" in placeholders else
                          tf.compat.v1.placeholder(tf.float32, [None] + model.policy_target_shape))
    ...
    model.assert_batched_shape("policy_target2", self.policy_target2, model.policy_target_shape)
    model.assert_batched_shape("policy_target_weight2", self.policy_target_weight2, model.policy_target_weight_shape)
    ...
    self.policy2_loss_unreduced = self.policy_target_weight2 * 0.15 * (
      tf.nn.softmax_cross_entropy_with_logits_v2(labels=self.policy_target2, logits=policy_output[:,:,2])
    )
    ...
    #Current Forbidden Locs
    self.ownership_loss_unreduced = 10.0 * self.scoring_target_weight * (
      tf.reduce_sum(
        tf.nn.softmax_cross_entropy_with_logits_v2(
    ...
    #Future Forbidden Locs
    self.scoring_loss_unreduced = 1.5 * self.scoring_target_weight * (
      tf.reduce_sum(
        tf.nn.softmax_cross_entropy_with_logits_v2(
          labels=tf.stack([(1+self.scoring_target)/2,(1-self.scoring_target)/2],axis=3),
          logits=tf.stack([scoring_output,-scoring_output],axis=3)
        ) * tf.reshape(model.mask_before_symmetry,[-1,model.pos_len,model.pos_len]),
        axis=[1,2]
      ) / model.mask_sum_hw
    )
    ...
    self.policy2_loss = tf.reduce_sum(self.target_weight_used * self.policy2_loss_unreduced, name="losses/policy2_loss")
    ...
    if for_optimization:
      ...
      self.opt_loss = (
        ...
        #0.5*self.scorebelief_pdf_loss +
        #0.5*self.scorebelief_cdf_loss +
        self.ownership_loss +
        self.scoring_loss +
        self.futurepos_loss +
        #self.seki_loss +
        #0.5*self.scorestdev_reg_loss +
...
class ModelUtils:
  ...
  def build_model_from_tfrecords_features(features,mode,print_model,trainlog,model_config,pos_len,batch_size,lr_scale=None,gnorm_clip_scale=None,num_gpus_used=1):
    ...
    policy_target2 = features["ptncm"][:,2,:]
    policy_target2 = policy_target2 / tf.reduce_sum(policy_target1,axis=1,keepdims=True)
    placeholders["policy_target2"] = policy_target2
    placeholders["policy_target_weight2"] = features["gtnc"][:,35]
    ...
    placeholders["scoring_target"] = features["vtnchw"][:,4]
