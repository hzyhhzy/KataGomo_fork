...
def model_fn(features,labels,mode,params):
  ...
  if mode == tf.estimator.ModeKeys.EVAL:
    ...
    eval_metric_ops={
      ...
      "p2loss": tf.compat.v1.metrics.mean(target_vars.policy2_loss_unreduced, weights=target_vars.target_weight_used),
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    (p2loss,p2loss_op) = moving_mean("p2loss",target_vars.policy2_loss_unreduced, weights=target_vars.target_weight_used)
    ...
    tf.summary.scalar('lastmoveLoss', p2loss)
    tf.summary.scalar('curForbiddenLoss', oloss)
    tf.summary.scalar('futureForbiddenLoss', sloss)
    ...
    ops = [
      train_step,
      p0loss_op,p1loss_op,p2loss_op,vloss_op,tdvloss_op,smloss_op,leadloss_op,vtimeloss_op,sbpdfloss_op,sbcdfloss_op,
